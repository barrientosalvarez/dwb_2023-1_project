...
public interface RepoCart extends JpaRepository<Cart, Integer>{
    ...
    @Query(value="SELECT * FROM cart WHERE cart_id=:cart_id AND gtin=:gtin AND status=1", nativeQuery=true)
    Cart findByRfcAndGtin(@Param("cart_id") Integer cart_id, @Param("gtin") String gtin);
